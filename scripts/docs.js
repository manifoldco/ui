/**
 * Autogenerates src/index.html with READMEs
 */

const { existsSync, readFileSync, writeFileSync } = require('fs');
const { resolve } = require('path');
const Showdown = require('showdown');

/* Settings */

const MARKDOWN_OPTIONS = {
  emoji: true,
  simpleLineBreaks: false,
  tables: true,
};
const HTML_BEFORE = `

  <!-- DO NOT EDIT! This is autogenerated from a README. -->
  <!-- Any changes here will be blown away. Edit the README instead. -->

`;
const HTML_AFTER = ``;

/* Methods */
function getComponentName(contents) {
  const match = contents.match(/BEGIN README [a-z-]+(?=\s)/);
  return match
    ? match[0]
        .replace('BEGIN README ', '')
        .trim()
        .toLowerCase()
    : '';
}

/* Init */
// Select: opening comment + anything (including newlines) + closing comment
const MAGIC_FORMULA = /<!-- BEGIN README [^]+?(?=<!-- END README -->)/g;
const converter = new Showdown.Converter(MARKDOWN_OPTIONS);

const indexHTML = resolve(__dirname, '..', 'src', 'index.html');
const original = readFileSync(indexHTML, 'utf8');
let withDocs = original;
const readmes = original.match(MAGIC_FORMULA);
if (Array.isArray(readmes)) {
  readmes.forEach(readme => {
    const component = getComponentName(readme);
    const filepath = resolve(__dirname, '..', 'src', 'components', component, 'readme.md');
    if (!existsSync(filepath)) {
      console.warn(`${filepath} not found`);
      return;
    }
    const md = readFileSync(filepath, 'utf8');
    const html = converter
      .makeHtml(md)
      .replace(
        '<hr />\n<p><em>Built with <a href="https://stenciljs.com/">StencilJS</a></em></p>',
        ''
      );
    // We included the opening comment in our selection, so let’s put it back
    const newContents = `<!-- BEGIN README ${component} -->
${HTML_BEFORE}
${html}
${HTML_AFTER}`;
    withDocs = withDocs.replace(readme, newContents);
  });
}

// Inject CSS

const CSS_BEFORE = `

  <!-- DO NOT EDIT! This is autogenerated from a CSS file. -->
  <!-- Any changes here will be blown away. Edit the CSS instead. -->

  <style>
`;
const CSS_AFTER = `</style>`;

function getStylesheetName(contents) {
  const match = contents.match(/BEGIN CSS [a-z.]+(?=\s)/);
  return match
    ? match[0]
        .replace('BEGIN CSS ', '')
        .trim()
        .toLowerCase()
    : '';
}

const CSS_MAGIC_FORMULA = /<!-- BEGIN CSS [^]+?(?=<!-- END CSS -->)/g;
const stylesheets = original.match(CSS_MAGIC_FORMULA);
if (Array.isArray(stylesheets)) {
  stylesheets.forEach(stylesheet => {
    const fileName = getStylesheetName(stylesheet);
    const filepath = resolve(__dirname, '..', 'src', 'global', fileName);

    if (!existsSync(filepath)) {
      console.warn(`${filepath} not found`);
      return;
    }
    const css = readFileSync(filepath, 'utf8');

    const newContents = `<!-- BEGIN CSS ${fileName} -->
${CSS_BEFORE}
${css}
${CSS_AFTER}`;
    withDocs = withDocs.replace(stylesheet, newContents);
  });
}
writeFileSync(indexHTML, withDocs, 'utf8');
